...
pub(super) unsafe fn bind(fd: &FdOps, addr: &sockaddr_un) -> io::Result<()> {
    let success = unsafe {
        let sock_size = size_of::<sockaddr_un>() as u32;
        let sock_size = sock_size.try_into().unwrap();
        libc::bind(
            fd.0,
...
pub(super) unsafe fn connect(fd: &FdOps, addr: &sockaddr_un) -> io::Result<()> {
    let success = unsafe {
        let sock_size = size_of::<sockaddr_un>() as u32;
        let sock_size = sock_size.try_into().unwrap();
        libc::connect(fd.0, addr as *const _ as *const _, sock_size) != -1
    };
    if success {
        Ok(())
    } else {
...
pub(super) fn set_passcred(fd: &FdOps, passcred: bool) -> io::Result<()> {
    #[cfg(uds_scm_credentials)]
    {
        ...
        let sock_size = size_of_val(&passcred) as u32;
        let sock_size = sock_size.try_into().unwrap();
        let success = unsafe {
            libc::setsockopt(
                fd.0,
