...
#compdef toney
compdef _toney toney

# zsh completion for toney                                -*- shell-script -*-
...
_toney()
{
    ...
    __toney_debug "CURRENT: ${CURRENT}, words[*]: ${words[*]}"
    ...
    __toney_debug "Truncated words[*]: ${words[*]},"

    lastParam=${words[-1]}
    lastChar=${lastParam[-1]}
    __toney_debug "lastParam: ${lastParam}, lastChar: ${lastChar}"

    # For zsh, when completing a flag with an = (e.g., toney -n=<TAB>)
    ...
    if [ "${lastChar}" = "" ]; then
        ...
        __toney_debug "Adding extra empty parameter"
        requestComp="${requestComp} \"\""
    fi

    __toney_debug "About to call: eval ${requestComp}"

    # Use eval to handle any environment variables and such
    out=$(eval ${requestComp} 2>/dev/null)
    __toney_debug "completion output: ${out}"
    ...
    __toney_debug "last line: ${lastLine}"
    ...
    if [ "${lastLine[1]}" = : ]; then
        ...
        __toney_debug "No directive found.  Setting do default"
        directive=0
    fi

    __toney_debug "directive: ${directive}"
    __toney_debug "completions: ${out}"
    __toney_debug "flagPrefix: ${flagPrefix}"

    if [ $((directive & shellCompDirectiveError)) -ne 0 ]; then
        __toney_debug "Completion received error. Ignoring completions."
        return
    fi
    ...
    while IFS='\n' read -r comp; do
        ...
        if [ "${comp[1,$endIndex]}" = "$activeHelpMarker" ];then
            __toney_debug "ActiveHelp found: $comp"
            comp="${comp[$startIndex,-1]}"
            if [ -n "$comp" ]; then
                compadd -x "${comp}"
                __toney_debug "ActiveHelp will need delimiter"
                hasActiveHelp=1
            fi
        ...
        if [ -n "$comp" ]; then
            ...
            __toney_debug "Adding completion: ${comp}"
            completions+=${comp}
            lastComp=$comp
        fi
    done < <(printf "%s\n" "${out[@]}")
    ...
    if [ $hasActiveHelp -eq 1 ]; then
        if [ ${#completions} -ne 0 ] || [ $((directive & shellCompDirectiveNoFileComp)) -eq 0 ]; then
            __toney_debug "Adding activeHelp delimiter"
            compadd -x "--"
            hasActiveHelp=0
        fi
    fi
    ...
    if [ $((directive & shellCompDirectiveNoSpace)) -ne 0 ]; then
        __toney_debug "Activating nospace."
        noSpace="-S ''"
    fi

    if [ $((directive & shellCompDirectiveKeepOrder)) -ne 0 ]; then
        __toney_debug "Activating keep order."
        keepOrder="-V"
    fi
    ...
    if [ $((directive & shellCompDirectiveFilterFileExt)) -ne 0 ]; then
        ...
        __toney_debug "File filtering command: $filteringCmd"
        ...
        if [ -n "$subdir" ]; then
            __toney_debug "Listing directories in $subdir"
            pushd "${subdir}" >/dev/null 2>&1
        else
            __toney_debug "Listing directories in ."
        fi
        ...
        __toney_debug "Calling _describe"
        if eval _describe $keepOrder "completions" completions $flagPrefix $noSpace; then
            __toney_debug "_describe found some completions"

            # Return the success of having called _describe
            return 0
        else
            __toney_debug "_describe did not find completions."
            __toney_debug "Checking if we should do file completion."
            if [ $((directive & shellCompDirectiveNoFileComp)) -ne 0 ]; then
                __toney_debug "deactivating file completion"
                ...
                __toney_debug "Activating file completion"
